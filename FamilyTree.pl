...
% Full name creation: combines two atoms into a string
...
% Root people - only Murat Aslan and Sedanur Aslan (Level 0)
level(Name, 0) :-
    member(Name, ['Murat Aslan', 'Sedanur Aslan']).

% Level calculation for children with blood ties - PRIORITY!
level(Name, Level) :-
    % If not root, continue
    \+ member(Name, ['Murat Aslan', 'Sedanur Aslan']),
   % Check if there is a blood relation (at least one of the parents is present)
    ...
    !. % Stop when blood relation is found, do not activate marriage rule

% For people related by marriage (groom-bride)
% ONLY for people NOT related by blood - SIMPLE VERSION
level(Name, Level) :-
    % If they are not root persons
    \+ member(Name, ['Murat Aslan', 'Sedanur Aslan']),
    % If this person is ABSOLUTELY not related by blood (parents are none or blank)
    person(_, Name, _, _, _, Father, Mother, _),
    (Father = none; Father = ''),
    (Mother = none; Mother = ''),
    % Find your match (family member)
    find_spouse(Name, FamilyMemberSpouse),
    FamilyMemberSpouse \= '',
    FamilyMemberSpouse \= none,
    % Find the level of your family member and place him/her in the same level  
...
% Age-comparative sibling relationships
...
% Close relatives control - marriage barriers
close_relative(P1, P2) :-
    1%. Same person control
    P1 = P2, !.
...
close_relative(P1, P2) :-
    4% Grandfather Grandmother-grandchild relationship
    (grandparent(P1, P2) ; grandparent(P2, P1)), !.

close_relative(P1, P2) :-
      %5. Uncle, maternal uncle, aunt, aunt-nephew relationship
...
% Cousin control (uncle/aunt's children)
cousins(C1, C2) :-
    parent(P1, C1),
    parent(P2, C2),
    same_parents(P1, P2),
...
% Add marriage (full version)
add_marriage :-
      %1. Get family member information
    ...
    %4. Get other information of new contact
    ...
    5%. Check the basic rules for marriage (age, gender)
    ...
    6%. RELATIVE CONTROL (MARRIAGE OBSTACLES)
...
;
   % Here forbidden marriage/2 is called, prints a message
  ...
  % Check if people are registered
...
% Marriage adding function - Improved version
add_marriage_improved :-
    ...
    (
        ...
        ;
       % If the person does not exist, get their information and add them
        write('Type new person birth year: '), flush_output,
       ...
       % We know the main person, additionally the other person's parent information is unknown
...
% Do not show contact information
show_person_info :-
...
% Relationship query (advanced Turkish version)
find_relationship :-
...
% Line where the program is started
start :- main_menu.

add_test_data :-
   % Murat and Sedanur's children (Level 1)
...
print_people_with_spouses([Person|Rest]) :-
    ...
    (Spouse == '' -> 
        format('~w~n', [Person])
    ; 
       % Press if its partner is not already pressed at another level
